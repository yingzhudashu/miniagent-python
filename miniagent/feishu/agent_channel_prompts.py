...
请如实说明：需在运行环境开启 `MINIAGENT_FEISHU_TOOLS=1`，或 **不设置** `MINIAGENT_FEISHU_TOOLS` 且设置 `MINIAGENT_FEISHU_TOOLS_AUTO=1`（并已配置 `FEISHU_APP_ID`/`SECRET`）；并配置开放平台权限与可选 `FEISHU_DOCX_URL_PREFIX` 等；详见仓库 `docs/FEISHU.md`「飞书工具与 IM 自检清单」。"""
