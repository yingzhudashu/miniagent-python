...
- **`feishu_send_interactive_card`** / **`feishu_update_message_card`**：发送或更新交互卡片；按钮 `value` 建议含 `miniagent_text`、`chat_id`，可选 `action_id`、`dedupe_key`（防连点，需 `MINIAGENT_FEISHU_CARD_ACTION_ROUTER=1`）。
...
请开启 `MINIAGENT_FEISHU_TOOLS=1`，或不要设置 `MINIAGENT_FEISHU_TOOLS` 且保持 `MINIAGENT_FEISHU_TOOLS_AUTO=1`（并已配置 `FEISHU_APP_ID`/`SECRET`）；详见 `docs/FEISHU.md`。"""
