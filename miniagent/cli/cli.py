...
``pip install`` 后的 ``miniagent`` 命令及 ``python -m miniagent.cli.cli`` 均指向本模块。
实际参数解析与启动逻辑在 ``miniagent.__main__``。

用法（与 ``python -m miniagent`` 等价）::

    miniagent                            # 交互模式（默认）
    miniagent --help                     # 显示命令行用法
    python -m miniagent.cli.cli --help   # 同上
...
def main() -> None:
    """CLI 打包入口，委托 ``miniagent.__main__.main``。
