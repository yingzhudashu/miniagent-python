...
本模块在 :mod:`miniagent.types.planning` 数据契约与 Phase 2 执行之间做轻量变换；
LLM JSON 的完整解析流程见 :func:`miniagent.core.planner._dict_to_plan`（含步骤规范化）。
...
def parse_plan_steps_from_raw(
    ...
    *,
    ...
    ``step_as_dict`` / ``step_thinking_level`` 由 :mod:`miniagent.core.planner` 注入，
    ...
    Returns:
        解析后的步骤列表（未经 :func:`miniagent.core.planner._normalize_plan_steps` 规范化）。
    """
    steps: list[PlanStep] = []
