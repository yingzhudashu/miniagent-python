...
2. **JSON 默认值种子** — 以 ``DEFAULT_*`` 命名；``config.py`` / ``get_config(key, constant)`` 将其作为
   ``config.defaults.json`` 未显式设置时的回退值（如 ``DEFAULT_AGENT_MAX_TURNS``）。
...
JSON 默认值种子应与本文件及 ``config.defaults.json`` 保持同步。
...
# 向后兼容别名（推荐新代码使用 DEFAULT_* 或从 AgentConfig 读取）
AGENT_MAX_TURNS = DEFAULT_AGENT_MAX_TURNS
AGENT_TOOL_TIMEOUT = DEFAULT_AGENT_TOOL_TIMEOUT
# 循环检测历史条数默认；对应 agent.loop_detection.history_size
...
FEISHU_PATCH_QUEUE_ENABLED = False  # 是否启用 patch 队列（默认直发）
FEISHU_PATCH_QUEUE_MAX_SIZE = 10  # patch 队列最大深度
...
HEARTBEAT_TIMEOUT = INSTANCE_HEARTBEAT_TIMEOUT  # 向后兼容别名
