...
:mod:`miniagent.agent.planner`、:mod:`miniagent.agent.plan_utils`、:mod:`miniagent.agent.agent`、
:mod:`miniagent.agent.executor`。
...
class PlanStep:
    ...
    ``depends_on`` 由 :func:`miniagent.agent.plan_utils.order_steps_by_dependencies` 拓扑排序。
...
class SuggestedConfig:
    ...
    Phase 1 生成，Phase 2 前由 :func:`miniagent.agent.agent._merge_plan_suggested_config`
    与默认配置、用户配置合并。
...
class StructuredPlan:
    ...
    由 LLM 根据用户需求和可用工具箱生成，经 :mod:`miniagent.agent.planner` 解析与规范化。
