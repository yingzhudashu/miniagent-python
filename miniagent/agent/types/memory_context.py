...
（``miniagent.assistant.memory.memory_context_service.DefaultMemoryContext``），
避免核心层反向依赖记忆层的具体实现。
...
```python
# executor.py - 核心层
from miniagent.agent.types.memory_context import MemoryContextProtocol
...
class MemorySearchProtocol(Protocol):
    ...
    实现类：``miniagent.assistant.memory.memory_context_service.DefaultMemorySearch``
    """
...
class MemoryHistoryProtocol(Protocol):
    ...
    实现类：``miniagent.assistant.memory.memory_context_service.DefaultMemoryHistory``
    底层格式化：``miniagent.agent.history.format_history_for_llm``
    """
