...
- ``ToolDefinition.permission``：工具元数据，描述该工具的安全级别与是否需要用户确认。
  ``require-confirm`` 由 :func:`miniagent.core.executor.execute_plan` 在调用 handler 前
...
- 默认实现：:class:`miniagent.infrastructure.registry.DefaultToolRegistry`、
  :class:`miniagent.memory.context.DefaultContextManager`
...
class Toolbox:
    ...
    与 ``miniagent.skills.builtin_toolboxes.BUILTIN_TOOLBOXES`` 及技能注册表中的
...
class RegisteredTool(ToolDefinition):
    ...
    ``name`` 由 :meth:`miniagent.infrastructure.registry.DefaultToolRegistry.register`
...
class TokenEstimate:
    ...
    由 :func:`miniagent.memory.context.estimate_token_estimate` 产生；
    :func:`miniagent.memory.context.estimate_tokens` 返回其中的 ``tokens`` 字段。
...
class ContextManagerProtocol(Protocol):
    ...
    默认实现：:class:`miniagent.memory.context.DefaultContextManager`
    在 token 超过阈值时执行压缩；``append`` 可能抛出
    :class:`miniagent.memory.context.ContextBudgetExceeded`（overflow_strategy=error）。
