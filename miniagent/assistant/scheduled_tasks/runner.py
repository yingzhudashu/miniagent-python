"""将单条 :class:`~miniagent.assistant.scheduled_tasks.models.ScheduledTask` 编译为可 ``await`` 的协程及队列 chat_id。
...
class _ScheduledJobRunner:
        ...
        try:
            reply = await self.ctx.engine.run_agent_with_thinking(
                ...
                client=getattr(
                    self.ctx,
                    "llm_client",
                    getattr(self.ctx, "llm_gateway", None),
                ),
    ...
    def _prompt(self, inbound: InboundMessage) -> str:
        ...
        from miniagent.agent.timezone import now_in_process_tz
