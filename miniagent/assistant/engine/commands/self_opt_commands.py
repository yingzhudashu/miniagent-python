...
from miniagent.agent.trace_events import ProposalStatus
...
async def handle_self_opt(
    ...
    **_kwargs: Any,
    ...
    """解析并执行一个自优化子命令。"""
    from miniagent.agent.constants import CLI_SELF_OPT_TOOLS
...
def cmd_self_opt_status() -> None:
    """打印自我优化开关、提案目录及待处理数量。"""
    from miniagent.assistant.infrastructure.json_config import get_config
    from miniagent.assistant.self_opt.proposal_store import ProposalStore, get_proposal_output_dir
...
def cmd_self_opt_report(date: str | None = None) -> None:
    ...
    from miniagent.assistant.self_opt.proposal_store import get_reports_dir
