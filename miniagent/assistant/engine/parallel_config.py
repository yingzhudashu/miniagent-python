...
本模块负责将 ``agent.parallel_sessions`` 映射到 :class:`~miniagent.assistant.infrastructure.message_queue.MessageQueueManager`
的跨队列串行策略；引擎层的 per-session 并行与限流由
:class:`~miniagent.assistant.engine.session_exec.SessionExecCoordinator` 单独处理（``agent.max_parallel_sessions``）。
"""
...
from miniagent.assistant.infrastructure.json_config import get_config_bool

if TYPE_CHECKING:
    from miniagent.assistant.contracts.runtime import ChannelRouterProtocol, MessageQueueProtocol
...
def configure_message_queue_for_parallel(message_queue: MessageQueueProtocol) -> None:
    ...
    与 :class:`~miniagent.assistant.engine.session_exec.SessionExecCoordinator` 的分工：
...
def resolve_active_session_key(
    ...
    :class:`~miniagent.assistant.infrastructure.channel_router.ChannelRouter` 返回 ``"__cli__"`` 本身。
