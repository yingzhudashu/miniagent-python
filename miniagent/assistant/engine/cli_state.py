...
``run_runtime`` 在进程启动时构造 :class:`CliLoopState` 字典，经闭包传递给 CLI 主循环、
...
if TYPE_CHECKING:
    from miniagent.agent.types.memory import SessionManagerProtocol
...
class CliLoopState(_CliLoopStateRequired, total=False):
    """主循环与 ``dispatch_command`` 使用的状态键（与 ``main.run_runtime`` 一致）。
