...
典型调用方参数（见 ``miniagent.assistant.engine.main``）：
...
async def _close_protocol_and_process_resources() -> None:
    ...
    try:
        from miniagent.assistant.mcp.runtime import close_mcp_connections
...
async def _close_llm_gateways(ctx: ApplicationContainer) -> None:
...
async def _close_network_resources(ctx: ApplicationContainer) -> None:
    ...
    try:
        await _close_llm_gateways(ctx)
    except Exception as error:
        _logger.debug("shutdown_runtime: LLM gateways close: %s", error, exc_info=True)
    from miniagent.assistant.feishu.drive_client import close_http_client
    ...
    from miniagent.assistant.tools.html_upload import close_html_upload_http_clients
...
def _begin_shutdown_trace() -> tuple[str, int, int]:
    ...
    from miniagent.agent.observability import emit_trace, new_trace_id
...
async def _finish_shutdown_trace(span_id: str, wall_start: int, cpu_start: int) -> None:
    ...
    from miniagent.agent.observability import emit_trace, shutdown_trace_writer
