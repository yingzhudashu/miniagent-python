...
class FeishuRuntime:
    """飞书 WebSocket 长轮询生命周期（绑定到特定 :class:`~miniagent.assistant.infrastructure.message_queue.MessageQueueManager`）。
    ...
    **关停**：优先 ``await stop_async()``（``shutdown_runtime``、CLI ``/feishu stop``）；
    ...
    def __init__(self, message_queue: Any) -> None:
        """Args:
        message_queue: 与 CLI 共用的 :class:`~miniagent.assistant.infrastructure.message_queue.MessageQueueManager`。
        """
    ...
    async def _run_poll_loop(
        ...
        from miniagent.assistant.feishu.im_tool_policy import log_feishu_im_tools_startup_hint_once
    ...
    def _report_started_mode() -> None:
        ...
        try:
            from miniagent.assistant.infrastructure.instance import update_instance_mode
