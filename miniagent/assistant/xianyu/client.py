    ...
    async def publish_item(
        ...
        *,
        ...
        if delivery == "distance_based":
            ...
            post_fee.update(templateId="0", postPriceInCent=_cents(shipping_fee or 0))
        elif delivery == "pickup_only":
            post_fee["templateId"] = "0"
        price: dict[str, str] = {}
        if current_price is not None:
            price["priceInCent"] = _cents(current_price)
        if original_price is not None:
            price["origPriceInCent"] = _cents(original_price)
        ...
        body = {
            ...
            "itemCatDTO": {
                "catId": str(prediction.get("catId")),
                "catName": str(prediction.get("catName")),
                "channelCatId": str(prediction.get("channelCatId")),
                "tbCatId": str(prediction.get("tbCatId")),
            },
            "uniqueCode": str(int(time.time() * 1000)),
