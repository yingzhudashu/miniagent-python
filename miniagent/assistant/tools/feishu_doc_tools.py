...
from miniagent.assistant.feishu._utils import fmt_json, resolve_under_workspace
...
def _trace_docx_render(
    ...
    from miniagent.agent.trace_events import EVENT_FEISHU_DOCX_RENDER
...
def _action_get(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    ...
    from miniagent.assistant.feishu.docx.client import get_document
...
def _action_read(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    """读取云文档正文内容与块类型统计。"""
    from miniagent.assistant.feishu.docx.blocks import list_document_blocks
    from miniagent.assistant.feishu.docx.client import get_document, get_document_raw_content
...
def _action_append(args: dict[str, Any], cfg: FeishuConfig, *, full_write: bool) -> ToolResult:
    ...
    from miniagent.assistant.feishu.docx.blocks import (
...
def _action_delete(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    """删除云文档。"""
    from miniagent.assistant.feishu.docx.client import delete_document
...
def _action_get_block(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    """获取单个内容块详情。"""
    from miniagent.assistant.feishu.docx.blocks import get_block
...
def _action_update_block(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    """更新单个内容块的文本内容。"""
    from miniagent.assistant.feishu.docx.blocks import update_block_text
...
def _action_delete_block(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    """删除单个内容块。"""
    from miniagent.assistant.feishu.docx.blocks import delete_block
...
def _action_export_raw(args: dict[str, Any], ctx: ToolContext, cfg: FeishuConfig) -> ToolResult:
    """导出云文档正文到本地文件。"""
    from miniagent.assistant.feishu.docx.client import get_document_raw_content
...
def _action_create_table(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    """在云文档中创建空白表格块。"""
    from miniagent.assistant.feishu.docx.tables import create_table_block
...
def _action_write_table_cells(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    """写入表格单元格内容。"""
    from miniagent.assistant.feishu.docx.tables import write_table_cells
...
def _action_create_table_with_values(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    """创建表格块并直接填充数据。"""
    from miniagent.assistant.feishu.docx.tables import create_table_with_values
...
def _action_upload_image(args: dict[str, Any], ctx: ToolContext, cfg: FeishuConfig) -> ToolResult:
    """上传本地图片到云文档并插入。"""
    from miniagent.assistant.feishu.docx.media import upload_doc_image_from_path
...
def _action_upload_file(args: dict[str, Any], ctx: ToolContext, cfg: FeishuConfig) -> ToolResult:
    """上传本地文件作为云文档附件素材。"""
    from miniagent.assistant.feishu.docx.media import upload_doc_file_from_path
...
def _action_download_media(args: dict[str, Any], ctx: ToolContext, cfg: FeishuConfig) -> ToolResult:
    """下载云文档素材（图片/附件）到本地文件。"""
    from miniagent.assistant.feishu.docx.media import download_media_bytes
...
async def _action_upload_image_from_message(
    ...
    """
    from miniagent.assistant.feishu.docx.media import upload_doc_image_from_bytes
...
def _action_copy(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    """复制云文档到指定文件夹。"""
    from miniagent.assistant.feishu.drive_extra import copy_file
    from miniagent.assistant.feishu.folder_token_resolve import resolve_parent_folder_token
...
def _action_move(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    """移动云文档到指定文件夹。"""
    from miniagent.assistant.feishu.drive_extra import move_file
    from miniagent.assistant.feishu.folder_token_resolve import resolve_parent_folder_token
...
def _action_list_permissions(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    """列出云文档协作者权限。"""
    from miniagent.assistant.feishu.drive_extra import list_permissions
...
def _action_search(args: dict[str, Any], cfg: FeishuConfig) -> ToolResult:
    """搜索云文档（按关键词或类型筛选）。"""
    from miniagent.assistant.feishu.drive_extra import (
        SearchApiError,
