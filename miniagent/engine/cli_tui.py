"""prompt_toolkit full-screen CLI composition and interaction loop."""
...
async def run_cli_loop(
    ...
    cli_outbound_dispatcher = OrderedOutboundDispatcher(outbound_channels)
    ctx.cli_outbound_dispatcher = cli_outbound_dispatcher
    ...
    _drag_start_x: list[int | None] = [None]  # 水平拖动起始 X 坐标
    _dragging_scrollbar = [False]  # 正在拖动垂直滚动条
    _drag_start_y = [0]  # 滚动条拖动起始 Y 坐标
    _SCROLLBAR_WIDTH = 2  # 滚动条宽度（右侧约 1-2 列）
    _transcript_window_ref: list[Any] = [None]  # Window 引用（用于设置 horizontal_scroll）
    ...
    class _TranscriptPaneControl(UIControl):
        ...
        def _handle_copy_mode_mouse(self, mouse_event: MouseEvent) -> NotImplementedOrNone:
            """复制模式下处理鼠标选择。
    ...
    def _on_enter(event):
        ...
        if text:
            ...
            if text.startswith("!"):
                ...
                if bash_cmd:
                    ok, output = run_cli_shell_command(bash_cmd)
    ...
    def _ensure_input_history_ready_for_nav() -> None:
        ...
        if len(input_buffer._working_lines) <= 1 and get_strings():
            sync_preload_buffer_working_lines(input_buffer)
    ...
    def _deliver_cli_final(session_key: str, text: str) -> None:
        """Render a final event through the session-aware TUI appenders."""
        from miniagent.engine.cli_format import format_cli_reply_block

        format_cli_reply_block(
            _transcript_coordinator.make_session_append(session_key),
            _transcript_coordinator.make_session_append_ansi(session_key),
            text,
            render_width=_rule_line_width(),
            markdown_width=_markdown_render_width(),
        )

    def _deliver_cli_error(_session_key: str, text: str) -> None:
        _append_transcript("class:cli-err", text)

    def _deliver_cli_status(_session_key: str, text: str) -> None:
        term_write(text)

    def _deliver_cli_thinking(event: OutboundEvent) -> None:
    ...
    outbound_channels.register(
        CliChannelAdapter(
            ...
            _deliver_cli_error,
            _deliver_cli_status,
            _deliver_cli_thinking,
        ),
        replace=True,
    )

    def _publish_cli_thinking(
        fragment: str,
        kind: str = "chunk",
        *,
        session_key: str = "",
        ...
        task = cli_outbound_dispatcher.publish(
            build_cli_thinking_event(
                fragment,
    ...
    engine.thinking.set_output_sink(_publish_cli_thinking)

    async def _process_input(message: InboundMessage) -> None:
        """Render one normalized CLI turn and execute it through the engine."""
        from miniagent.engine.cli_format import format_cli_user_block
        ...
        try:
            ...
            async with engine.session_turn(session_key):
                try:
                    _cli_rule_heavy()
                    was_at_bottom = _output_at_bottom()
                    _stick_bottom[0] = True
                    try:
                        _snap_output_bottom()
                        get_app().invalidate()
                    ...
                    await cli_outbound_dispatcher.drain(target)
    ...
    while True:
        ...
        if user_input.startswith("/"):
            ...
            if reply is not None:
                ...
                command_session_key = resolve_active_session_key(
                    channel_router,
                    state.get("active_session_id") or "default",
                )
                await outbound_channels.send(
        ...
        # ── Agent 执行 ──
        message = build_cli_inbound_message(user_input, active_sk, interface="tui")
        await inbound_turns.submit(message, _process_input)
