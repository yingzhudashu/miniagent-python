...
def test_default_receive_id_open_id_uses_injected_sender(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MINIAGENT_FEISHU_RECEIVE_ID_TYPE", "open_id")
    ctx = ToolContext(
        cwd="/tmp",
...
def test_default_receive_id_open_id_missing_sender_errors(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MINIAGENT_FEISHU_RECEIVE_ID_TYPE", "open_id")
...
def test_default_receive_id_explicit_arg_overrides(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MINIAGENT_FEISHU_RECEIVE_ID_TYPE", "open_id")
    ctx = ToolContext(
        cwd="/tmp",
...
def test_effective_receive_id_type_arg_over_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MINIAGENT_FEISHU_RECEIVE_ID_TYPE", "open_id")
