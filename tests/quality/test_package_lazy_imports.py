...
def test_core_agent_import_succeeds_in_fresh_process() -> None:
    result = _fresh_import("import miniagent.agent.agent")
...
def test_engine_package_does_not_eagerly_import_cli_or_runtime() -> None:
    result = _fresh_import(
        "import miniagent.assistant.engine",
        "assert 'miniagent.assistant.engine.main' not in sys.modules",
        "assert 'miniagent.assistant.engine.cli_tui' not in sys.modules",
    )
...
def test_memory_package_does_not_eagerly_import_embedding_stack() -> None:
    result = _fresh_import(
        "import miniagent.assistant.memory",
        "assert 'miniagent.assistant.memory.embedding_search' not in sys.modules",
    )
...
def test_knowledge_context_import_does_not_eagerly_import_yaml_stack() -> None:
    result = _fresh_import(
        ...
        "assert 'miniagent.assistant.knowledge.registry' not in sys.modules",
        "assert 'miniagent.assistant.knowledge.base' not in sys.modules",
...
def test_knowledge_registry_lazy_export_remains_compatible() -> None:
    result = _fresh_import(
        "from miniagent.assistant.knowledge import KnowledgeRegistry",
...
def test_memory_lazy_export_remains_compatible() -> None:
    result = _fresh_import(
        "from miniagent.assistant.memory import KeywordIndex",
...
def test_types_config_does_not_import_openai_sdk() -> None:
    result = _fresh_import(
        "import miniagent.agent.types.config",
...
def test_single_tool_module_does_not_import_all_tool_collections() -> None:
    result = _fresh_import(
        "import miniagent.assistant.tools.data_tools",
        "assert 'miniagent.assistant.tools.feishu_doc_tools' not in sys.modules",
        "assert 'miniagent.assistant.tools.filesystem' not in sys.modules",
    )
