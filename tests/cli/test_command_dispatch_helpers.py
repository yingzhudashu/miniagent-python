...
def test_last_qa_reads_current_session_history_and_handles_failures() -> None:
    ...
    assert _get_last_qa(manager, "s") == ("question", "answer")
    assert _get_last_qa(SimpleNamespace(get=lambda _sid: None), "s") == (None, None)
    session.conversation_history = [{"role": "user", "content": "only"}]
    assert _get_last_qa(manager, "s") == (None, None)


def test_last_qa_loader_and_failure() -> None:
    ...
    assert _get_last_qa(manager, "s") == ("loaded question", "loaded answer")
...
async def test_review_empty_clean_and_iterative(monkeypatch) -> None:
    ...
    assert await _run_review("q", "a", capture=True) is None
    assert await _run_review("q", "a", capture=True) is None
    result = await _run_review("q", "a", extra_feedback="more", capture=True)
...
async def test_improve_success_failure_and_term_sink(monkeypatch) -> None:
    ...
    assert await _run_improve("q", "a", [], capture=True) is None


def test_capture_and_status_busy_bindings() -> None:
    assert "boom" in capture_output(lambda: (_ for _ in ()).throw(RuntimeError("boom")))
...
async def test_run_test_capture_and_real_mode_validation(monkeypatch) -> None:
    ...
    output = await _run_test(capture=True)
    assert "1/2" in output and "case" in output
    assert "需要 registry" in await _run_test(mock=False, capture=True)
...
def test_list_samples_and_last_status(monkeypatch) -> None:
    ...
    assert "sample" in _list_test_samples()
    assert "50.0%" in _get_test_status()
    fake.load_samples = lambda: []
    fake.get_last_report = lambda: None
    assert "暂无" in _list_test_samples()
    assert "暂无" in _get_test_status()
