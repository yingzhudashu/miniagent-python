...
def test_extract_tool_intent_truncation_suffix(monkeypatch: pytest.MonkeyPatch) -> None:
    from miniagent.agent.executor import _extract_tool_intent, _reset_env_caches_for_tests

    monkeypatch.setattr("miniagent.agent.executor.EXECUTION_TOOL_INTENT_MAX_CHARS", 8)
...
def test_extract_tool_intent_zero_means_no_clip(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("miniagent.agent.executor.EXECUTION_TOOL_INTENT_MAX_CHARS", 0)
    _reset_env_caches_for_tests()
    long_cmd = "y" * 500
    s = _extract_tool_intent("exec_command", {"command": long_cmd})
