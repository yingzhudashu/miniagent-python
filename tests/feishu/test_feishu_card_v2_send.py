...
def test_normalize_lark_md_heading_to_bold(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert normalize_lark_md("# 一级标题") == "**一级标题**"
    assert normalize_lark_md("### 三级标题") == "**三级标题**"
    assert normalize_lark_md("###### 六级标题") == "**六级标题**"
    # 正文中的 ### 不应被转换
    assert "### text" in normalize_lark_md("some ### text")
