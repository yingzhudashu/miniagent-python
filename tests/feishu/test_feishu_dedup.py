"""Tests for transactional Feishu message deduplication."""
...
def test_empty_message_id_is_not_persisted(tmp_path: Path) -> None:
    dedup = FeishuDeduplicator(str(tmp_path))
    assert dedup.try_begin_processing("  ") is True
    assert dedup.stats() == {
        "processing_claims": 0,
        "disk_dedup": 0,
        "dirty": False,
