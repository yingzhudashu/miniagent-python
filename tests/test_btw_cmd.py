...
def manager() -> BackgroundTaskManager:
    """Return an isolated manager; no test relies on process-global state."""
    instance = BackgroundTaskManager()
    yield instance
    if instance._cleanup_task is not None:
...
class TestCmdBtwStatus:
    ...
    def test_status_nonexistent_task(self, manager):
        ...
        result = cmd_btw_status(manager, "nonexistent123")
        assert "不存在" in result
        assert ERROR_PREFIX in result

    def test_status_empty_list(self, manager):
        """cmd_btw_status without ID shows empty list message."""
        # Clear any existing tasks first
        cmd_btw_clear(manager)

        result = cmd_btw_status(manager)
        assert "没有后台任务" in result or "任务列表" in result

    def test_status_with_id_format(self, manager):
        """cmd_btw_status with ID returns formatted output."""
        result = cmd_btw_status(manager, "nonexistent")
...
class TestCmdBtwClear:
    ...
    def test_clear_empty(self, manager):
        ...
        result = cmd_btw_clear(manager)
        assert "没有需要清理" in result or "已清理" in result

    def test_clear_returns_string(self, manager):
...
class TestCmdBtwStart:
    ...
    async def test_start_with_mock_engine(self, manager):
        ...
        result = await cmd_btw_start(manager, engine, "Test prompt", state)
    ...
    async def test_start_short_prompt_no_ellipsis(self, manager):
        ...
        result = await cmd_btw_start(manager, MockEngine(), "hi", {"skill_toolboxes": []})
        assert "输入: hi" in result
        assert "hi..." not in result

    @pytest.mark.asyncio
    async def test_start_at_concurrent_limit(self, manager):
        ...
        result = await cmd_btw_start(manager, engine, "Should fail", state)
...
class TestCmdBtwResult:
    ...
    @pytest.mark.asyncio
    async def test_result_failed_task(self, manager):
        ...
        start_result = await cmd_btw_start(manager, engine, "fail me", state)
        import re
        ...
        result = await cmd_btw_result(manager, task_id)
        assert "错误" in result
...
class TestCmdBtwCancel:
    ...
    @pytest.mark.asyncio
    async def test_cancel_running_task_stays_cancelled(self, manager):
        ...
        start_result = await cmd_btw_start(manager, engine, "slow", state)
        import re
        ...
        cancel_result = await cmd_btw_cancel(manager, task_id)
        assert "已取消" in cancel_result

        await asyncio.sleep(0.2)
        status_result = cmd_btw_status(manager, task_id)
...
class TestIntegration:
    ...
    async def test_full_workflow(self, manager):
        ...
        # Start task
        start_result = await cmd_btw_start(manager, engine, "Integration test", state)
        ...
        if task_id:
            ...
            # Clear
            clear_result = cmd_btw_clear(manager)
