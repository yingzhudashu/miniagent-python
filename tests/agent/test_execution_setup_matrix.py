...
from miniagent.agent.execution_runtime_setup import resolve_exec_tools, step_thinking_header
...
def test_resolve_exec_tools_obeys_strategy_precedence() -> None:
    registry = _Registry()
    assert resolve_exec_tools(registry, _config("all"), _plan(enabled=False), None) == []
    assert resolve_exec_tools(registry, _config("all"), _plan(), None) == ["all"]
    assert resolve_exec_tools(
        registry,
        _config("auto"),
        _plan(toolboxes=["plan"]),
        SimpleNamespace(required_toolboxes=["step"]),
    ) == ["boxes:step"]
    assert resolve_exec_tools(registry, _config("auto"), _plan(), None) == ["core"]
    assert resolve_exec_tools(_Registry(core=False), _config("auto"), _plan(), None) == ["all"]
    assert resolve_exec_tools(
        registry,
        _config("manual"),
...
def test_step_thinking_header_uses_fallback_and_truncates_description() -> None:
    ...
    header = step_thinking_header(0, 1, SimpleNamespace(step_number=7, description="x" * 80))
