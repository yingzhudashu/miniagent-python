...
class TestFuzzyCommandMatching:
    ...
    def test_fuzzy_match_typo(self) -> None:
        """测试错别字匹配。"""
        from miniagent.assistant.engine.command_dispatch import _find_closest_command
    ...
    def test_prefix_match(self) -> None:
        """测试前缀匹配。"""
        from miniagent.assistant.engine.command_dispatch import _find_command_by_prefix
    ...
    def test_registered_commands_list(self) -> None:
        """验证命令列表完整。"""
        from miniagent.assistant.engine.command_dispatch import _REGISTERED_COMMANDS
...
class TestHttpRetry:
    ...
    def test_http_retry_import(self) -> None:
        """验证 HTTP 重试工具可导入。"""
        from miniagent.assistant.infrastructure.http_retry import (
...
class TestConfigWatch:
    ...
    def test_config_watch_import(self) -> None:
        """验证配置监听工具可导入。"""
        from miniagent.assistant.infrastructure.config_watch import (
            start_config_watch,
        )
...
class TestSetupWizard:
    ...
    def test_setup_wizard_import(self) -> None:
        """验证配置引导工具可导入。"""
        from miniagent.assistant.engine.setup_wizard import (
...
class TestOpenAiClientTimeout:
    ...
    def test_llm_gateway_timeout_config(self) -> None:
