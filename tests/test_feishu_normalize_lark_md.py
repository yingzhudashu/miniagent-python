...
def test_chunk_feishu_normalizes_before_split(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    raw = "\u200b# Title\n\n" + ("paragraph\n" * 80)
    chunks = ps._chunk_feishu_card_markdown(raw)
    assert len(chunks) >= 2
...
def test_prepare_card_markdown_skips_normalize_when_requested() -> None:
    ...
    no_norm = _prepare_card_markdown(raw, max_len=10_000, normalize=False)
    assert "\uff0a" in with_norm
...
def test_finalize_pipeline_matches_streaming_thinking_prep(
    ...
    acc = "段落一\n\n段落二\n\n**工具**\n\n- a\n- b"
    prep = ps._prepare_thinking_body_for_card(acc, apply_cap=False)
    chunks = ps._chunk_feishu_card_markdown(prep, already_normalized=True)
    assert len(chunks) == 1
