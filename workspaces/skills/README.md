...
- **`install_skill`** 工具安装成功后会自动调用 `refresh_skills` 加载到当前进程。
- 手工拷贝/修改技能目录后，在 CLI 或飞书执行 **`.reload-skills`** 全量重扫。
- 开发时可设 **`MINIAGENT_SKILLS_WATCH=1`**，引擎会监视技能目录并在变更后 debounce 自动 refresh。

**子会话**：热加载只更新主空间工具注册表；已在其它会话中克隆的工具不会自动撤回或升级，必要时新建会话或使用 promote 机制。
