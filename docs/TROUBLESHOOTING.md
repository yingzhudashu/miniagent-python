...
### ⚠️ Windows 全屏 CLI 只显示阶段标题

**症状**：任务难度、评估与计划或执行步骤只出现标题，没有对应正文。

**处理**：升级到包含 TUI 流式正文降级修复的版本。全屏 CLI 会在 Rich/ANSI 渲染不可用、返回空结果或解析失败时自动显示原始纯文本；该降级只作用于失败的正文事件。若状态栏显示“推理折叠”，切换为展开后会立即恢复当前阶段已累计的正文。

可设置 `AGENT_DEBUG=1` 后检索 `TUI thinking正文降级为纯文本`，确认触发原因；日志仅记录原因、平台和会话标识，不记录正文。
...
| 队列阻塞 / 卡住 | [运行问题](#运行问题) |
| Windows 全屏 CLI 只有阶段标题 | [运行问题 §Windows 全屏 CLI](#️-windows-全屏-cli-只显示阶段标题) |
