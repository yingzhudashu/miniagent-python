...
> Mini Agent Python | 版本: 5.0.0 | 最后更新: 2026-08-11 | 与 `miniagent.__version__` 对齐 | 命令使用 `/` 前缀；多数命令在 CLI 与飞书均可使用
...
**图片视觉描述**：
- 默认启用 `cli.file_vision_desc`（见 `miniagent/assistant/resources/config.defaults.json`）
...
重新加载 `config.user.json`。活动回合继续使用启动时取得的不可变快照；模型、凭据和多数运行参数从下一回合起生效。MCP 启动命令等只在进程初始化时装配的能力仍需重启：
...
CLI 思考块标题与正文默认使用亮青色（`ansibrightcyan`）。颜色为 **Internal 常量**，定义在 `miniagent/agent/constants.py`（`CLI_STYLE_THINK_HEAD` / `CLI_STYLE_THINK_BODY`）；**暂不支持** `config.user.json` 或环境变量覆盖。定制需修改源码常量；若未来实现 `cli.styles` 配置项，将在此文档化。
...
- 模型 Gateway：用户 JSON 使用 **`llm.max_retries`**（默认 2）；它控制 MiniAgent 的 provider 请求重试上限，不等同于第三方 SDK 的内部参数
- `agent.http_timeout`：默认 120 秒（包内 defaults，**这是**合法 JSON 键）

**覆盖模块**：
- LLM provider 调用（配置键 `llm.max_retries`；SDK 可能另有内部重试）
- Embedding 搜索
- ClawHub 客户端
- 飞书 Drive API
...
配置文件可在不中断当前回合的情况下重新加载；新快照从下一回合或下一次读取对应配置时生效。启动期装配项仍需重启：
