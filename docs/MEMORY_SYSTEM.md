...
| 组件 | 路径 / 行为 |
|------|-------------|
| **会话历史** | `workspaces/sessions/<safe_id>/history.json`，含 `user` / `thinking` / `assistant`；`thinking` 在调用 LLM 前由 `conversation_history_for_llm()` 映射为合法 `assistant` 文本块。默认 `memory.thinking_for_llm_mode=compact`，按 `memory.thinking_for_llm_compact_max_chars` 截断摘要；`full` 模式才使用 `memory.thinking_for_llm_max_chars`，且**不修改磁盘原文**。 |
...
| **Dream 式维护** | `workspaces/memory/dream_state.json` + `dream.lock`；周期默认 7d / 30d / 365d（JSON 配置 `dream.diary_refine_sec` / `dream.session_lt_refine_sec` / `dream.agent_lt_refine_sec`），体量超 `dream.size_force_bytes` 时可跳过周期闸门 |
...
`ground_truth_facts` 用于保存可作为后续需求澄清依据的长期稳定事实。它与 `key_facts` 的区别是：`key_facts` 是兼容旧版本的摘要字符串，`ground_truth_facts` 是带 key、状态、置信度和证据的结构化事实。
...
通过 `config.defaults.json` → `config.user.json` 的 `embedding.*` 控制（默认关闭，仅使用关键词索引）：
...
| `max_turns` | 400 | ReAct 最大轮数（与 `AgentConfig` 一致） |
...
> - `MINIAGENT_EMBEDDING_*` → `embedding.*`
> - `MINI_AGENT_DREAM_*` → `dream.*`
> - 其他变量请参考 `config.defaults.json` 的 `memory`、`embedding`、`dream` 节
>
...
| `MINIAGENT_EMBEDDING_ENABLED` | `0` | `embedding_search.py` 是否启用嵌入搜索 | `embedding.enabled` |
| `MINIAGENT_EMBEDDING_MAX_ENTRIES` | `2000` | `embedding_search.py` 嵌入条目上限 | `embedding.max_entries` |
| `MINI_AGENT_DREAM_*` | `7d/30d/365d` | `dream_scheduler.py` 维护周期 | `dream.diary_refine_sec` / `dream.session_lt_refine_sec` / `dream.agent_lt_refine_sec` |
| `MINI_AGENT_DREAM_SIZE_BYTES` | *(无)* | 体量闸门阈值 | `dream.size_force_bytes` |
