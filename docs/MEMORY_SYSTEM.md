...
> Mini Agent Python | 版本: 2.1.0 | 最后更新: 2026-07-11 | 与 `miniagent.__version__` 对齐 | 模块: `miniagent/memory/`
...
> ⚠️ **配置类 env 已废弃**：自 v2.0.0 起，**用户配置**仅通过 JSON（`config.user.json` > `config.defaults.json`），不再支持用 `MINIAGENT_*` 覆盖 defaults 中的配置键。
>
> **运维 / 路径类 env 仍有效**（如 `MINIAGENT_PATHS_STATE_DIR`、`AGENT_DEBUG`），完整三分类见 [ENGINEERING.md](ENGINEERING.md) §1.2。
>
> 以下列表为**历史配置类**变量参考，**不再生效**。请使用 `config.user.json` 中对应的配置项替代。
>
> **迁移示例**：`MINIAGENT_MEMORY_STORE_CACHE_MAX` → `memory.store_cache_max`；`MINIAGENT_REGISTRY_MAX_ENTRIES` → `memory.registry_max_entries`；`MINIAGENT_MEMORY_KEYWORD_INDEX_MAX` → `memory.keyword_index_max`。完整对照见 `config.defaults.json` 的 `memory`、`embedding`、`dream` 节与 [CHANGELOG.md](../CHANGELOG.md) Breaking changes。
