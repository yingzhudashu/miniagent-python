...
记忆相关参数在 `config.user.json` 的 `memory` / `agent` / `embedding` 节配置（默认值见 [`config.defaults.json`](../config.defaults.json)）：
...
> ⚠️ **配置类 env 已废弃**：自 v2.0.0 起，用户配置仅通过 JSON（`config.user.json` > `config.defaults.json`）。**运维 / 路径类 env 仍有效**（如 `MINIAGENT_PATHS_STATE_DIR`、`AGENT_DEBUG`），完整分类见 [ENGINEERING.md §1.2](ENGINEERING.md#12-环境变量分类)。
>
...
>
> 完整对照见 `config.defaults.json` 的 `memory`、`embedding`、`dream` 节与 [CHANGELOG.md](../CHANGELOG.md) Breaking changes。
