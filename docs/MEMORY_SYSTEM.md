...
**运行时注入**：正式入口通过 [`miniagent/assistant/memory/runtime.py`](../miniagent/assistant/memory/runtime.py) 构造唯一 `MemoryRuntime`，其中共享同一个注册表、关键词索引、嵌入索引、存储、活动日志和上下文服务；[`ApplicationContainer.memory`](../miniagent/assistant/bootstrap/application.py) 统一持有它，`UnifiedEngine` / `run_agent` / `execute_plan` 只接受显式注入，不存在模块级默认 bundle。状态根由 `MINIAGENT_PATHS_STATE_DIR` 或 `resolve_project_state_dir()` 决定（默认 `{miniagent}/workspaces/projects/{project_key}/`）。
...
│  ┌───────────────────────────────────────────┐  │
│  │ miniagent/assistant/memory/store.py                   │  │
...
│  ┌───────────────────────────────────────────┐  │
│  │ miniagent/assistant/memory/activity_log.py                │  │
...
│  ┌───────────────────────────────────────────┐  │
│  │ miniagent/assistant/memory/keyword_index.py               │  │
│  │ - 关键词索引 + TF-IDF 加权                  │  │
...
**位置**: `miniagent/assistant/memory/activity_log.py`
...
**位置**: `miniagent/assistant/memory/shared_registry.py`
...
**位置**: `miniagent/assistant/memory/keyword_index.py`
...
**位置**: `miniagent/assistant/memory/embedding_search.py`
...
记忆相关参数在 `config.user.json` 的 `memory` / `agent` / `embedding` 节配置（默认值见 [`miniagent/assistant/resources/config.defaults.json`](../miniagent/assistant/resources/config.defaults.json)）：
