...
```bash
python -m pytest -x                                      # 快速失败
python -m pytest tests/session                           # 单个能力模块
python -m pytest -k "test_register"                     # 匹配名称
```
...
| 源文件 | 测试文件 |
|--------|---------|
| `miniagent/agent/tools/registry.py` | `tests/tools/test_registry.py` |
| `miniagent/assistant/session/manager.py` | `tests/session/test_session.py` |
| `miniagent/assistant/security/sandbox.py` | `tests/tools/test_sandbox.py` |
