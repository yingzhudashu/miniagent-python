...
Mini Agent Python 的 **Agent 问答流水线**为 Phase 0 分类 → Phase 0.5 需求澄清 → Phase 1 规划 → Phase 2 执行 → Phase 3 反思，并通过 ReAct 循环按需调用工具。四模块是唯一源码分层；下图中的入口、渠道、工具、记忆和安全仅表示 `assistant` 内部产品子系统，不是额外的包级架构层。
...
基于 Claude 最佳实践，所有系统提示词统一管理在 `miniagent/agent/prompts/` 目录下：
...
| `cli_dispatch_tools.py` | `run_dot_command`：经 [`command_dispatch.dispatch_command`](../miniagent/assistant/engine/command_dispatch.py) 执行斜杠命令（`capture=True`，与 CLI 同源） |
...
**run_dot_command 与进程状态**：[`UnifiedEngine.run_agent_with_thinking`](../miniagent/assistant/engine/engine.py) 将共享 [`CliLoopState`](../miniagent/assistant/engine/cli_state.py) 写入 `AgentConfig.feishu_config.cli_loop_state`，[`execute_plan`](../miniagent/agent/executor.py) 再注入 `ToolContext`。飞书入站路径下默认 `cli_dispatch_allow_mutations=False`（与飞书里直接发 `/session` / `/schedule` 变异一致）；**`feishu.dot_commands_full=true`** 时为 True，与 CLI 同等。若嵌入代码只调用 [`run_agent`](../miniagent/agent/agent.py) 而不经 `run_agent_with_thinking`，需在 `agent_config.feishu_config` 中自行传入 `cli_loop_state`（及按需的 `cli_dispatch_allow_mutations`），否则工具会返回不可用说明。注册开关：**`cli.dot_tools_enabled`**（默认开启，`false` 跳过注册，见包内 defaults）。
...
位于 `miniagent/assistant/engine/` 与 `miniagent/assistant/infrastructure/`：命令模糊匹配、Tab 补全、HTTP 重试、配置热更新等。模块表与交互细节见 **[CLI.md §用户体验增强](CLI.md#用户体验增强)**。
...
### Assistant 内部契约、安全与 Agent 端口

- **Assistant 契约** (`assistant/contracts/`): 平台无关的 `InboundMessage` / `OutboundEvent` 与服务生命周期协议
- **Assistant 应用用例** (`assistant/application/`): `InboundTurnCoordinator` 接收标准消息并复用队列策略；`ChannelRegistry` 选择具体通道
- **Assistant 启动** (`assistant/bootstrap/`): `LifecycleManager` 负责启动、逆序回滚与统一关停

- **安全能力** (`assistant/security/sandbox.py`): 路径白名单、父目录遍历拦截、权限策略
- **Agent 类型与端口** (`agent/types/`, `agent/ports/`): 定义 Agent、Config、Plan、Tool、Skill、Memory 类型与注入接口
...
4. **用户入口**：终端与 CLI 侧 **`/schedule`** 子命令（`every` / `once` / **`cron`** 五段表达式，实现见 [`cli_commands.py`](../miniagent/assistant/engine/cli_commands.py)）；Agent 可选 **`manage_scheduled_task`**（含 `add_cron`，[`schedule_tools.py`](../miniagent/assistant/tools/schedule_tools.py)）；下一触发时间由 [`cron.py`](../miniagent/assistant/scheduled_tasks/cron.py) + **croniter** 计算。
...
配置热重载先用当前配置路径创建严格解析的候选 `JsonConfigLoader`，再用候选值构造新的 `LLMGateway`；两者都成功后才发布 loader、secrets 环境桥接与 Gateway。失败时当前配置和连接池保持不变。被替换的 Gateway 进入 `ApplicationContainer.retired_llm_gateways`，统一关停时与当前 Gateway 一起关闭，因此在途请求不会被热重载中断。
