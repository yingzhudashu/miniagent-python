...
> **功能层 vs 源码子包**：本文「12 个功能层」为逻辑分层（入口、引擎、核心、记忆等）；[`README.md`](../README.md) 项目结构中的「17 个子包」为 `miniagent/` 下的物理包划分，二者维度不同。
...
- **配置层级**：`config.defaults.json`（随代码发布）→ `config.user.json`（用户覆盖）。用户配置项**不支持** `MINIAGENT_*` 覆盖 defaults；运维/调试类 env 见 [ENGINEERING.md](ENGINEERING.md) §1.2。
...
2.x 迁移说明（顶层 `src` 移除、记忆包级导出弃用等）见 [CHANGELOG.md](../CHANGELOG.md) Breaking changes；飞书 `poll_server` WSClient 进程内复用见上文「已知技术债」。
