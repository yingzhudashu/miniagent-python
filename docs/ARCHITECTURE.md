...
> Mini Agent Python | 版本: 2.1.0 | 最后更新: 2026-07-11 | 与 `miniagent.__version__` 对齐 | 架构图: [architecture.drawio](architecture.drawio)
...
> **功能层 vs 源码子包**：本文「12 个功能层」为逻辑分层（入口、引擎、核心、记忆等）；[`INDEX.md`](INDEX.md) 目录树中的「17 个子包」为 `miniagent/` 下的物理包划分，二者维度不同。
...
**CLI 显示隔离**（与路由并行）：一般模式下群聊 Agent 仍运行但不写入全屏 transcript；群聊聚焦（CLI 绑定 `feishu:oc_*`）时仅镜像当前群。门控在 `miniagent/engine/main.py` 飞书 handler 入站侧实现，思考镜像经 `feishu_mirror_cli` 与引擎对齐。详见 [FEISHU.md §CLI 显示策略](FEISHU.md#cli-显示策略cli_feishu_policy)。
...
| 文件 | 职责 |
|------|------|
| `poll_server.py` | WebSocket 长连接：WSClient 单例、入站防抖合并（`message_debounce.py`）、与 `ws_health` 监督衔接（去重见 `feishu_dedup.py`） |
...
- **类型层** (`types/`): 8 个类型模块，定义 Agent、Config、Plan、Tool、Skill、Memory、Feishu 类型
  ...
  - 默认实现：`memory/memory_context_service.py`（`DefaultMemoryContext`），由 `RuntimeContext.memory_context` 注入
...
**飞书侧**：与 `/session` 类似，通常仅允许 `/schedule list` / `show`；`add` / `remove` / `enable` / `disable` 须在本地 CLI 执行（详见 [README.md](../README.md) 与 [USER_GUIDE.md](USER_GUIDE.md) §9）。
...
启动时由 `compat.unified_entry`（或等价入口）实例化 `RuntimeContext`（见 `runtime/context.py`）；`unified_entry` 会先从 `config.user.json` 加载 secrets 到环境变量，构造后调用 `set_runtime_context(ctx)`，进程关停时 `shutdown_runtime` 清空登记。仅嵌入 `unified_main(ctx)` 时，调用方须自行 `load_secrets_from_project_root()` 或设置 env，并视需要自行 `set_runtime_context(ctx)`。字段包括 `registry`、`monitor`、`skill_registry`、`clawhub`、`engine`，以及 **`channel_router`**、**`message_queue`**、**`feishu`**（`FeishuRuntime`），以及 **`memory_store`**、**`activity_log`**、**`keyword_index`**，以及 **`openai_client`**（入口通常设为 `get_shared_async_openai()`；为 `None` 时执行链回落共享工厂）。`unified_main`、CLI 主循环与飞书消息处理器通过该对象（或由其闭包捕获）获取依赖；`get_runtime_context()` 仅作无法改签名时的只读回退，新代码仍应显式传 `ctx`。
